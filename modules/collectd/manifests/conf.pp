...
define collectd::conf (
  ...
  $managed_mode = $mode ? {
    undef   => $collectd::file_mode,
    default => $mode,
  }

  $managed_owner = $owner ? {
    undef   => $collectd::file_owner,
    default => $owner,
  }

  $managed_group = $group ? {
    undef   => $collectd::file_group,
    default => $group,
  }

  $managed_require = $require ? {
    undef   => $collectd::file_require,
    default => $require,
  }

  $managed_notify = $notify ? {
    undef   => $collectd::file_notify,
    default => $notify,
  }

  $managed_replace = $replace ? {
    undef   => $collectd::file_replace,
    default => $replace,
  }
